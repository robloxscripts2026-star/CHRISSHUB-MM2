--[[
    CHRISHUB V2 - THE ULTIMATE NEON FIX
    -------------------------------------------
    - UI: Azul Neón Futurista (Estilo Lista Limpia)
    - FIX: Kill Aura 45 Studs (Ultra Agresivo)
    - FIX: TP Sheriff (Auto-pickup Gun)
    - FIX: Aimbot Murderer con FOV Ajustable
    - COLORS: Lista directa para cambiar colores
    -------------------------------------------
]]
...
function ShowKeySystem()
    ...
    B.MouseButton1Click:Connect(function() for _,k in pairs(CH_V2.Keys) do if I.Text == k then KeyF:Destroy(); ShowMain() return end end; B.Text = "KEY ERROR"; task.wait(1); B.Text = "ACCEDER" end)
end
...
function ShowMain()
    ...
    AddToggle(ComP, "AUTO-SHOT", "AUTOSHOT")
    AddToggle(ComP, "KILL AURA (45ST)", "KILLAURA")
    AddToggle(ComP, "TP SHERIFF GUN", "TPSHERIFF")

    -- Ajuste de FOV (Slider Simple)
    local FOVLabel = Instance.new("TextLabel", ComP); FOVLabel.Size = UDim2.new(1,-10,0,20); FOVLabel.Text = "TAMAÑO FOV: " .. CH_V2.Settings.FOV_SIZE; FOVLabel.TextColor3 = Color3.new(1,1,1); FOVLabel.BackgroundTransparency = 1; FOVLabel.TextSize = 10
    ...
    FOVSlider.MouseButton1Click:Connect(function()
        ...
        FOVLabel.Text = "TAMAÑO FOV: " .. CH_V2.Settings.FOV_SIZE
    end)

    -- [ COLORES - ESTILO LISTA QUE PEDISTE ]
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= lp and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if CH_V2.Settings.KILLAURA and role == "Murderer" then
                    ...
                    if dist < 45 then
                        -- Solo ataca si tienes el cuchillo o para defenderte si eres Sheriff
                        local myKnife = lp.Character:FindFirstChild("Knife") or lp.Backpack:FindFirstChild("Knife")
                        if myKnife then
                            lp.Character.HumanoidRootPart.CFrame = hrp.CFrame * CFrame.new(0, 0, 2)
                            -- Intenta atacar si es posible
                            pcall(function() myKnife.Stab:FireServer() end)
                        end
                    end
                end

                -- Aimbot Murderer (Inside FOV)
